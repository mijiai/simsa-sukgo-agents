...
- [ ] 1-2-1. Naver Search API 클라이언트 모듈 작성 (`httpx` 비동기)
- [ ] 1-2-2. 기업명 기반 뉴스 검색 함수 구현 (최신순, 페이지네이션)
- [ ] 1-2-3. 뉴스 결과 파싱 및 정규화 (제목 · 본문 · 날짜 · URL 추출)
- [ ] 1-2-4. 부정 키워드 필터링 로직 구현 (소송, 횡령, 부도, 적자 등)
- [ ] 1-2-5. API Rate Limit 처리 및 재시도 로직 (exponential backoff)
