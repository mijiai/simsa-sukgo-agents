...
8. 인과관계 중심 서술: 단순 현상 나열이 아닌 '원인-현상-결과-향후 영향'의 구조로 작성한다[cite: 43, 44, 49].
9. 금융 전문 용어 활용: '현금창출력 제약', '금융비용 점증', '재무안정성 저하', '가격전가력 보유' 등 정제된 금융 용어를 사용합니다.
10.보수적 관점 유지: 긍정적 지표 뒤에는 반드시 잠재적 리스크(고정비 부담, 투자 소요 등)를 검토하여 중립적이고 보수적인 결론을 도출합니다.
11.정량 데이터의 정성적 해석: 숫자를 언급할 때는 그것이 기업의 실질적 상환 능력에 미치는 '의미'를 반드시 포함합니다.
"""
